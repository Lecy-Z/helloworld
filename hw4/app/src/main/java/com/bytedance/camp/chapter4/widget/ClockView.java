...
public class ClockView extends View {
    ...
    private Paint hneedlePaint = new Paint();
    private Paint mneedlePaint = new Paint();
    private Paint sneedlePaint = new Paint();
    ...
    private void init() {
        ...
        hneedlePaint.setAntiAlias(true);
        hneedlePaint.setColor(Color.WHITE);
        hneedlePaint.setStrokeWidth(HOUR_NEEDLE_WIDTH);
        hneedlePaint.setStrokeCap(Paint.Cap.ROUND);
        hneedlePaint.setStyle(Paint.Style.STROKE);

        mneedlePaint.setAntiAlias(true);
        mneedlePaint.setColor(Color.WHITE);
        mneedlePaint.setStrokeWidth(MINUTE_NEEDLE_WIDTH);
        mneedlePaint.setStrokeCap(Paint.Cap.ROUND);
        mneedlePaint.setStyle(Paint.Style.STROKE);

        sneedlePaint.setAntiAlias(true);
        sneedlePaint.setColor(Color.WHITE);
        sneedlePaint.setStrokeWidth(SECOND_NEEDLE_WIDTH);
        sneedlePaint.setStrokeCap(Paint.Cap.ROUND);
        sneedlePaint.setStyle(Paint.Style.STROKE);
    ...
    private void drawTimeNeedles(Canvas canvas) {
        ...
        float dh=(hour+(float)minute/60)*360/12;
        float dm=(minute+(float)(second/60))*6;
        int ds=second*360/60;

        Log.i("time",hour+"时"+minute+"分"+second+"秒");
        //Log.i("time","dm:"+dm);

        //canvas.save();
        //秒
        float dsendX = (float) (centerX + radius * SECOND_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(ds)));
        float dsendY = (float) (centerY - radius * SECOND_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(ds)));
        canvas.drawLine(centerX,centerY,dsendX,dsendY,sneedlePaint);
        //min
        float dmendX = (float) (centerX  +radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(dm)));
        float dmendY = (float) (centerY - radius * MINUTE_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(dm)));
        canvas.drawLine(centerX,centerY,dmendX,dmendY,mneedlePaint);
        //hour
        float dhendX = (float) (centerX + radius * HOUR_NEEDLE_LENGTH_RATIO * Math.sin(Math.toRadians(dh)));
        float dhendY = (float) (centerY - radius * HOUR_NEEDLE_LENGTH_RATIO * Math.cos(Math.toRadians(dh)));
        canvas.drawLine(centerX,centerY,dhendX,dhendY,hneedlePaint);
